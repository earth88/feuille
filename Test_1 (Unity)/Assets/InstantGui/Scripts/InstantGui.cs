...
public class InstantGuiActivator
{
	...
	public void  Activate ( MonoBehaviour control)
	{
		...
		if (message != null && message.Length != 0 && (Application.isPlaying || messageInEditor)) {
			...
			case MessageRecievers.current:
				control.gameObject.SendMessage (message, control, SendMessageOptions.DontRequireReceiver);
				break;
			case MessageRecievers.upwards:
				control.gameObject.SendMessageUpwards (message, control, SendMessageOptions.DontRequireReceiver);
				break;
			case MessageRecievers.broadcast:
				control.gameObject.BroadcastMessage (message, control, SendMessageOptions.DontRequireReceiver);
				break;
			case MessageRecievers.gameObject:
				if (messageGameObject != null){
					Debug.Log (messageGameObject+" "+message+" "+parameter);
					messageGameObject.SendMessage (message, parameter, SendMessageOptions.DontRequireReceiver);
				}
				break;
			case MessageRecievers.all: 
				GameObject[] allObjs = (GameObject[])GameObject.FindObjectsOfType (typeof(GameObject));
				for (int i=0; i<allObjs.Length; i++)
					allObjs [i].SendMessage (message, control, SendMessageOptions.DontRequireReceiver);
				break;
			}
		}
	}
